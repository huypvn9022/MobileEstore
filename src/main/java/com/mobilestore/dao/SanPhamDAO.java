...
public interface SanPhamDAO extends JpaRepository<SanPham, Integer>{
	...
	@Query(value = "SELECT o FROM SanPham o WHERE o.donGia  BETWEEN ?1 and ?2 ")
	Page <SanPham> findByDongia(Pageable pageable , Double  min, Double  max);
			
	@Query(value = "select o from SanPham o where o.mach.ram like ?1 ")
	Page <SanPham> findAllByRam(Pageable pageable,String ram);
	
	@Query(value = "select o from SanPham o where o.mach.rom like ?1")
	Page <SanPham> findAllByRom(Pageable pageable,String rom);

	
	@Query(value = "select o from SanPham o where o.mach.heDH like ?1")
